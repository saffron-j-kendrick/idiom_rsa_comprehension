...
def get_final_mod_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="final_modifier_no_context", torch_device="cuda", save_attention=False):
...
def get_final_head_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="final_head_no_context", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_final_word_token_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence_and, layers, load_if_available=True, batch_size = 1, rep_type="final_word_no_context", rep_loc='./data', torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_noun_noun_compound_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="idiom_no_context", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_mean_pooled_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="mean_pooled_no_context", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def extract_and_save_representations(amount_of_dataset=1, batch_size=1, models=None, load_if_available=False, layers=None, torch_device="cpu", representations=["mean_pooled"], rep_loc='./data', save_attention=False):
    ...
    mod_head_words_per_sentence_with_context = data_utils.get_idiom_modifier_head_words_per_sentence_no_context()
    head_and_words_per_sentence_with_context = data_utils.get_idiom_modifier_head_words_per_sentence_no_context_and()
    b1_b2_words_per_sentence_with_context = data_utils.get_idiom_modifier_head_words_per_sentence_no_context_bs()
    ...
    b1_b2_words_per_sentence_with_context = b1_b2_words_per_sentence_with_context[:num_to_keep]
    ...
    for model_name in tqdm.tqdm(models):
        ...
        if "final_modifier_no_context" in representations:
        ...
        if "mean_pooled" in representations:
            ...
            attention_mask = inputs["attention_mask"]
            #input_ids, attention_mask = unpack_dict(tokeniser(sentences.tolist(), max_length=512, return_tensors='pt', pad_to_max_length=True))
            get_mean_pooled_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, layers=layers, load_if_available=load_if_available, batch_size=batch_size, rep_type='mean_pooled_no_context', rep_loc=rep_loc, torch_device=torch_device, save_attention=save_attention)
